...
const ByTime: React.FC<MyComponentProps> = () => {
  ...
  const paragraphs = truncatedText.split("\n").map((line, index) => (
    <p
      key={index}
      className="leading-6 tracking-normal text-black font-normal text-[16px] w-fit mt-[17px]"
    >
      {line}
    </p>
  ));
  ...
  return (
    <Page>
      <div className="w-full h-full bg-[#f4eee3] scrollbar-hide">
        {/* top */}
        ...
        <div className="w-fit mx-auto -translate-y-[206px]">
          {Object.values(HoroscopeByDay).map((timeTaken, key) => (
            <button
              id={key.toString()}
              className={`text-[#9f7c35] text-[16px] mx-1 font-medium px-4 py-3 border-2 border-[#9f7c35] rounded-md ${
        ...
        <div className="flex items-center pl-6 font-medium -translate-y-[216px] text-[#9f7c35] text-[16px] leading-loose space-x-3">
