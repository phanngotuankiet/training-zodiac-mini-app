...
const ByTime: React.FC<MyComponentProps> = () => {
  ...
  const paragraphs = truncatedText.split('\n').map((line, index) => (
    <p key={index} className="leading-6 tracking-normal text-black font-normal text-[16px] poppins w-fit mt-[17px]">
      {line}
    </p>
  ));
    ...
    <Page>
      <div className="w-full bg-[#f4eee3] overflow-x-scroll scrollbar-hide">
        ...
        <div className="w-fit mx-auto -translate-y-[206px]">
          <ContainerForEachSign zodiacName={"Bảo Bình"}
            zodiacSVG={<AquariusSVG />} />
        ...
        <div className="flex poppins items-center pl-6 font-medium -translate-y-[216px] text-[#9f7c35] text-[16px] leading-loose space-x-3"><p>Xem thêm</p> {arrowIcon}</div>
        <Footer />
      </div >
      <Footer />
    </Page >
